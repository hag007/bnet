...
def main():
    ...
    parser.add_argument('--metric_folder', dest='metric_folder', default=constants.config_json["metric_folder"])
    ...
    for dataset_file in dataset_files:
        ...
        for comb in combs:
            tuning_comb
            params.append([dataset_name, algo, true_solutions_folder, comb])
            combs_str += "_" + "_".join([dataset_name, algo] + [str(tuning_comb[a]) for a in \
    ...
    metrics_folder = os.path.join(true_solutions_folder, "metrics")
...
def fetch_metrics(args):
    dataset_name, algo, true_solutions_folder, comb = args
