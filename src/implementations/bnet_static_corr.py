...
def get_edge_weight(edge):
    # 1/(1+(np.abs(G_modularity.get_edge_data(edge[0], edge[1])["score"])*5))
    w=1-np.abs(G_modularity.get_edge_data(edge[0], edge[1])["score"])
    open("/specific/netapp5/gaga/hagailevi/omics/output/weights/corr_weights.txt", 'a+').write(f'{w}\n')
    return w
...
def main(active_genes_file, network_file, slices_file=None, slice_threshold=0.3, module_threshold=0.05,
    ...
    # print("start running bnet_static_corr...")
