...
from src.implementations.bnet_static_corr import main as bnet_main
...
class DominoStaticCorrRunner(AbstractRunner):
    def __init__(self):
        super().__init__("DOMINO_STATIC_CORR")
    ...
    def run(self, dataset_file_name, network_file_name, output_folder, **kwargs):
        print("run bnet_static_corr runner...")
        ...
        if 'use_cache' in kwargs:
            constants.USE_CACHE=False # kwargs['use_cache']=='true'
        slice_threshold = 0.3
        ...
        if 'cancer_type' in kwargs:
            cancer_type = kwargs['cancer_type']



        network_file_name=os.path.splitext(network_file_name)[0]+f"_{cancer_type}.txt"
